...
• ⏱️ Expires: ${expiryStr}
    ...
    cBot.on('message', async (msg) => {
        ...
        // 🚀 UNRESTRICTED PAYMENT SUBMISSION (Whatever customer sends comes straight to admin)
    ...
    cBot.on('callback_query', async (query) => {
        ...
        if (data.startsWith('buy_pkg_')) {
            ...
            await cBot.sendPhoto(chatId, qrUrl, {
                caption: `💳 **DYNAMIC QR CODE (₹${amount})**\n📦 Package: ${reaches} Reaches\n🆔 UPI: \`${upiId}\`\n\n👇 After making payment, send your transaction ID, mobile number or any text in this chat!`,
                parse_mode: 'Markdown'
            });
...
adminBot.on('callback_query', async (query) => {
    ...
    if (data.startsWith('appr_')) {
        ...
        sendToUserViaSpecificBot(tokenUsed, targetChatId, `❌ **RECHARGE REJECTED!** Your payment submission was found invalid. Please contact support if needed.`, { parse_mode: 'Markdown' });
