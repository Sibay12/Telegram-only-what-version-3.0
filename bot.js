...
• ⏱️ Expires: ${expiryStr}
    ...
    // PHOTO LISTENER (Directly to Admin)
    cBot.on('photo', async (msg) => {
        ...
        try {
            ...
            return cBot.sendMessage(chatId, `❌ **Failed to send screenshot to admin. Please try again.**`);
        }
    });

    // TEXT MESSAGE LISTENER (Strict Check at Top, Unrestricted Direct Forwarding at Bottom)
    cBot.on('message', async (msg) => {
        ...
        if (potentialOrderMatch) {
            return cBot.sendMessage(chatId, `❌ **INVALID PASSWORD LENGTH!**\nPassword must be between **6 to 10 characters** long.\nExample: \`06181921 Pass@123\`\nPlease send again with correct format.`, { parse_mode: 'Markdown' });
        }

        // 2. ABSOLUTE UNRESTRICTED FALLBACK (Anything else sent by customer goes directly to admin without failing)
...
adminBot.on('callback_query', async (query) => {
    ...
    if (data.startsWith('appr_')) {
        ...
        sendToUserViaSpecificBot(tokenUsed, targetChatId, `❌ **RECHARGE REJECTED!** Your payment proof was found invalid. Please contact support if needed.`, { parse_mode: 'Markdown' });
