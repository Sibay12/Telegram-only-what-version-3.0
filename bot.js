...
// -------------------------------------------------------------
// 🎧 SUPPORT BOT ENGINE (Fully Fixed Routing & Replies)
// -------------------------------------------------------------
...
supportBot.on('message', async (msg) => {
    ...
    // If message is sent by Admin directly in Support Bot chat
...
• ⏱️ Expires: ${expiryStr}
    ...
    cBot.on('message', async (msg) => {
        ...
        // Check if message is a Target Order (10-digit ID + Password)
        ...
        // 🔄 FLEXIBLE TRANSACTION ID / UTR REDIRECT (Catches any text/number sent after QR scan)
        const utr = text.trim();
