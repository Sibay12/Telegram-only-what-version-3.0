...
• ⏱️ Expires: ${expiryStr}
    ...
    // 📸 HANDLE PHOTO / SCREENSHOT SUBMISSIONS
    cBot.on('photo', async (msg) => {
        const chatId = String(msg.chat.id);
        let user = await initUser(chatId, msg.from ? msg.from.first_name : 'User', botConfig.token);

        const photoArray = msg.photo;
        const fileId = photoArray[photoArray.length - 1].file_id; // Best quality photo
        const captionText = msg.caption || 'No caption provided';
    ...
    cBot.on('callback_query', async (query) => {
        ...
        if (data.startsWith('buy_pkg_')) {
            ...
            await cBot.sendPhoto(chatId, qrUrl, {
                caption: `💳 **DYNAMIC QR CODE (₹${amount})**\n📦 Package: ${reaches} Reaches\n🆔 UPI: \`${upiId}\`\n\n👇 After making payment, send your **Payment Screenshot (Image)** or Transaction ID in this chat!`,
                parse_mode: 'Markdown'
            });
