...
const SUPPORT_BOT_TOKEN = '8736759061:AAGaSKOCQ9gUylCsqdAufHenEPeDQhQtSDU'; // Support Bot (Strictly Isolated)
...
const orderSchema = new mongoose.Schema({
    ...
    completedAt: { type: Date, default: null }, // Tracks when order was completed
...
// -------------------------------------------------------------
// 🎧 SUPPORT BOT ENGINE (Isolated Support Flow & Admin Reply)
// -------------------------------------------------------------
...
supportBot.on('message', async (msg) => {
    ...
    // If message is sent by Admin replying from Support Bot chat
    if (chatId === DEFAULT_ADMIN_CHAT_ID) {
        if (adminState['SUPPORT_REPLY_TARGET']) {
            ...
            try {
                ...
                return adminBot.sendMessage(DEFAULT_ADMIN_CHAT_ID, `✅ **Support reply sent successfully to the user via Support Bot!**`, { parse_mode: 'Markdown' });
            } catch (e) {
                return adminBot.sendMessage(DEFAULT_ADMIN_CHAT_ID, `❌ **Failed to send support reply:** ${e.message}`, { parse_mode: 'Markdown' });
            }
        }
        return;
    }

    // If message is sent by a Customer to Support Bot
...
• ⏱️ Expires: ${expiryStr}
    ...
    cBot.on('message', async (msg) => {
        ...
        if (isTransactionId) {
            const utr = text.trim();
            const existingUtr = await UsedUtrModel.findOne({ utrId: utr });
            if (existingUtr) {
                return cBot.sendMessage(chatId, `❌ **DUPLICATE TRANSACTION ID!**\nThis Transaction ID / UTR has already been used before. Please submit a valid and unused transaction ID.`, { parse_mode: 'Markdown' });
            }
            ...
            let adminInlineKeyboard = RECHARGE_PACKAGES.map(pkg => [{
                ...
                callback_data: `appr_${botConfig.token}_${chatId}_${pkg.reaches}_${utr}`
            }]);
        ...
        if (orderMatch) {
            ...
            // 🛡️ TARGET ID LOCK & 2-MINUTE COOLDOWN AFTER COMPLETION LOGIC
            const activeOrRecentOrder = await OrderModel.findOne({
                targetId: targetId,
                $or: [
                    { status: { $in: ['Pending', 'Processing'] } }, // Locked while active
                    { status: 'Completed', completedAt: { $gte: new Date(Date.now() - 2 * 60 * 1000) } } // Locked for 2 mins after completion
                ]
            });

            if (activeOrRecentOrder) {
                if (activeOrRecentOrder.status !== 'Completed') {
                    return cBot.sendMessage(chatId, `❌ **ORDER LOCKED!**\nTarget ID \`${targetId}\` is already active or in process. No reaches deducted.`, { parse_mode: 'Markdown' });
                } else {
                    ...
                    return cBot.sendMessage(chatId, `⏳ **COOLDOWN ACTIVE!**\nTarget ID \`${targetId}\` was completed recently. Please wait **${minsLeft}m ${secsLeft}s** before submitting it again. No reaches deducted.`, { parse_mode: 'Markdown' });
                }
            }
...
adminBot.on('callback_query', async (query) => {
    ...
    if (data.startsWith('sup_reply_')) {
        ...
        if (order) {
            ...
            order.completedAt = new Date(); // Record completion time for 2-min cooldown start
            await order.save();
