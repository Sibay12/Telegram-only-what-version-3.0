...
const fs = require('fs');
const axios = require('axios'); // Required for Instamojo API request
...
• ⏱️ Expires: ${expiryStr}
    ...
    cBot.onText(/\/buy/, async (msg) => {
        ...
        let pkgText = `🏷️ **SELECT A RECHARGE PACKAGE:**\n\nPlease click on any package below to get the Instamojo Payment Link & QR Code:`;
        let inlineKeyboard = [];
...
• ⏱️ Expires: ${expiryStr}
    ...
    cBot.on('photo', async (msg) => {
        ...
        let adminInlineKeyboard = RECHARGE_PACKAGES.map(pkg => [{
            ...
            callback_data: `appr_${botConfig.token}_${chatId}_${pkg.reaches}_SCREENSHOT`
        }]);
        ...
        try {
            ...
            return cBot.sendMessage(chatId, `✅ **Payment Screenshot Successfully Submitted!**\nThe payment card has been removed. Reaches and validity will be updated once verified by the admin.`, {
                parse_mode: 'Markdown',
    ...
    cBot.on('message', async (msg) => {
        ...
        // 2. UNIVERSAL UPI / UTR REFERENCE VERIFICATION
        const cleanedText = text.trim();
        const upiRefMatch = cleanedText.match(/^[a-zA-Z0-9]{10,25}$/);

        if (upiRefMatch) {
            const utrId = cleanedText;
            const existingUtr = await UsedUtrModel.findOne({ utrId: utrId });
            
            if (existingUtr) {
                return cBot.sendMessage(chatId, `❌ **Auto Error Remark:** This UTR / UPI Reference Number has already been used before. Please enter a valid unique transaction reference.`, { parse_mode: 'Markdown' });
            }
            ...
            let adminInlineKeyboard = RECHARGE_PACKAGES.map(pkg => [{
                text: `✅ Approve +${pkg.reaches} Reaches (₹${pkg.amount})`,
                callback_data: `appr_${botConfig.token}_${chatId}_${pkg.reaches}_${utrId}`
            }]);
            adminInlineKeyboard.push([{ text: `❌ Reject Submission`, callback_data: `rej_${botConfig.token}_${chatId}` }]);

            await adminBot.sendMessage(DEFAULT_ADMIN_CHAT_ID, `💳 **NEW INSTAMOJO / UPI REFERENCE SUBMITTED**\n\n🤖 Bot Source: \`${botConfig.id}\`\n👤 Customer: ${user.firstName} (\`${chatId}\`)\n💰 Remaining Wallet: \`${user.reaches} Reaches\`\n🔢 Reference ID: \`${utrId}\``, {
                parse_mode: 'Markdown',
                reply_markup: { inline_keyboard: adminInlineKeyboard }
            });

            return cBot.sendMessage(chatId, `✅ **Reference Number Successfully Submitted!**\nThe payment card has been removed. Reaches and validity will be updated once verified by admin.`, {
                parse_mode: 'Markdown',
                reply_markup: { remove_keyboard: true }
            });
        }

        // 3. ABSOLUTE UNRESTRICTED FALLBACK
        user.pendingUtrAmount = null;
        await user.save();

        let adminInlineKeyboardFallback = RECHARGE_PACKAGES.map(pkg => [{
            text: `✅ Approve +${pkg.reaches} Reaches (₹${pkg.amount})`,
            callback_data: `appr_${botConfig.token}_${chatId}_${pkg.reaches}_NONE`
        }]);
        adminInlineKeyboardFallback.push([{ text: `❌ Reject Submission`, callback_data: `rej_${botConfig.token}_${chatId}` }]);

        await adminBot.sendMessage(DEFAULT_ADMIN_CHAT_ID, `💳 **NEW PAYMENT / SUBMISSION RECEIVED**\n\n🤖 Bot Source: \`${botConfig.id}\`\n👤 Customer: ${user.firstName} (\`${chatId}\`)\n💰 Remaining Wallet: \`${user.reaches} Reaches\`\n💬 Content Sent:\n"${cleanedText}"`, {
            parse_mode: 'Markdown',
            reply_markup: { inline_keyboard: adminInlineKeyboardFallback }
        });

        return cBot.sendMessage(chatId, `✅ **Your submission has been sent to the admin successfully!**\nThe payment card has been removed. Reaches and validity will be updated once verified.`, {
            parse_mode: 'Markdown',
            reply_markup: { remove_keyboard: true }
        });
    });

    // INSTAMOJO PAYMENT LINK & QR GENERATOR CALLBACK
    cBot.on('callback_query', async (query) => {
        ...
        if (data.startsWith('buy_pkg_')) {
            ...
            const loadingMsg = await cBot.sendMessage(chatId, `⏳ **Generating Instamojo Payment Link & QR Code for ₹${amount}...**`, { parse_mode: 'Markdown' });

            try {
                // Call Instamojo Live API to create payment request
                const params = new URLSearchParams();
                params.append('purpose', `JPW Reach Recharge - ${reaches} Reaches`);
                params.append('amount', amount);
                params.append('buyer_name', query.from.first_name || 'Customer');
                params.append('send_email', 'False');
                params.append('send_sms', 'False');
                params.append('redirect_url', 'https://t.me/' + botConfig.token); // or your success handler

                const response = await axios.post(INSTAMOJO_ENDPOINT, params, {
                    headers: {
                ...
                const paymentUrl = paymentData.longurl;
                const paymentRequestId = paymentData.id;

                // Generate QR code pointing directly to Instamojo payment link
                const qrUrl = `https://api.qrserver.com/v1/create-qr-code/?size=250x250&data=${encodeURIComponent(paymentUrl)}`;

                try { await cBot.deleteMessage(chatId, loadingMsg.message_id); } catch (e) {}

                await cBot.sendPhoto(chatId, qrUrl, {
                    caption: `💳 **INSTAMOJO LIVE PAYMENT (₹${amount})**\n📦 Package: ${reaches} Reaches\n\n👉 **Click the button below to pay directly via Instamojo:**\n\n👇 After making payment, send your **UPI Reference Number / UTR** or Screenshot in this chat!`,
                    parse_mode: 'Markdown',
                    reply_markup: {
                        inline_keyboard: [
                            [{ text: `🔗 Pay ₹${amount} via Instamojo`, url: paymentUrl }],
                            [{ text: '🔄 Refresh / Select Other Plan', callback_data: 'buy_pkg_refresh' }]
                        ]
                    }
                });
                ...
                try { await cBot.deleteMessage(chatId, loadingMsg.message_id); } catch (e) {}
                await cBot.sendMessage(chatId, `❌ **Auto Error Remark:** Failed to generate Instamojo payment link. Please try again later or contact support.`);
            }

            cBot.answerCallbackQuery(query.id);
        }
        else if (data === 'buy_pkg_refresh') {
            try { await cBot.deleteMessage(chatId, query.message.message_id); } catch (e) {}
            let pkgText = `🏷️ **SELECT A RECHARGE PACKAGE:**\n\nPlease click on any package below to get the Instamojo Payment Link & QR Code:`;
            let inlineKeyboard = [];
            RECHARGE_PACKAGES.forEach(pkg => {
            ...
            await cBot.sendMessage(chatId, pkgText, { parse_mode: 'Markdown', reply_markup: { inline_keyboard: inlineKeyboard } });
            cBot.answerCallbackQuery(query.id);
        }
    });
});
...
adminBot.on('callback_query', async (query) => {
    ...
    if (data.startsWith('appr_')) {
        ...
        if (utrId && utrId !== 'SCREENSHOT' && utrId !== 'NONE') {
            try { await UsedUtrModel.create({ utrId: utrId }); } catch (err) {}
        }
