...
// Used UTRs to prevent reuse across all UPI apps
...
• ⏱️ Expires: ${expiryStr}
    ...
    cBot.on('message', async (msg) => {
        ...
        // 2. UNIVERSAL UPI / UTR REFERENCE VERIFICATION (Google Pay, PhonePe, Paytm, BHIM, etc.)
        const cleanedText = text.trim();
        // Matches typical UPI reference / UTR numbers (alphanumeric numbers usually 10 to 22 characters long)
        ...
        if (upiRefMatch) {
            ...
            if (existingUtr) {
                return cBot.sendMessage(chatId, `❌ **Auto Error Remark:** This UTR / UPI Reference Number has already been used before across UPI apps. Please enter a valid unique transaction reference.`, { parse_mode: 'Markdown' });
            }
            ...
            await adminBot.sendMessage(DEFAULT_ADMIN_CHAT_ID, `💳 **NEW UPI / UTR REFERENCE SUBMITTED**\n\n🤖 Bot Source: \`${botConfig.id}\`\n👤 Customer: ${user.firstName} (\`${chatId}\`)\n💰 Remaining Wallet: \`${user.reaches} Reaches\`\n🔢 Reference ID: \`${utrId}\``, {
                parse_mode: 'Markdown',
                reply_markup: { inline_keyboard: adminInlineKeyboard }
            });

            return cBot.sendMessage(chatId, `✅ **UPI Reference Number Successfully Submitted!**\nThe QR card has been removed. Reaches and validity will be updated once verified by admin.`, {
                parse_mode: 'Markdown',
                reply_markup: { remove_keyboard: true }
            });
        }

        // 3. ABSOLUTE UNRESTRICTED FALLBACK (Any other text/message sent directly to admin)
        user.pendingUtrAmount = null;
        await user.save();
